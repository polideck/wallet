...
router.post("/getNonce", async (req, res) => {
  ...
  if (validator.isAddress(address, true)) {
    ...
    const accountInfo = await mongo
      .db("exilirate")
      .collection("users")
      .findOne({ eAddr: address });
    ...
    res
      .status(400)
      .json({ nonce: null, msg: "Invalid Request: Invalid Public Key" });
  }
});
  ...
  if (validator.isAddress(address)) {
    ...
    if (nonce == storedNonce && nonce != undefined) {
      ...
      if (valid) {
        ...
        res
          .status(400)
          .json({ jwt: null, msg: "Invalid Request: Invalid Signature" });
      }
    } else {
      //Error if nonce is invalid
      res
        .status(400)
        .json({ jwt: null, msg: "Invalid Request: Invalid Nonce" });
    }
  } else {
    //Error if public key is invalid
    res
      .status(400)
      .json({ jwt: null, msg: "Invalid Request: Invalid Public Key" });
  }
});
